...
function showPopUpMessage(content, success) {
  ...
  const message = createAndAppendElement("article", [
    "message",
    success ? "is-primary" : "is-warning",
  ]);
...
async function availableUsersDisplay() {
  const availableUsersList = document.querySelector("#availableUsers");
  while (availableUsersList.firstChild) {
    availableUsersList.removeChild(availableUsersList.firstChild);
  }
  try {
    ...
    users.forEach((user) => {
      ...
      li.classList.add("title", "is-5");
      li.textContent = `${user.id} : ${user.name}`;
      availableUsersList.append(li);
    });
  } catch (err) {
    console.log(err);
  }
      ...
      groups.data.forEach((group) => {
        ...
        listItem.addEventListener("click", () => {
          ...
          axios
            ...
            .then((result) => {
              document.querySelector("#groupNameDisplay").innerText =
                group.name;
              showMessagesOnScreen(result.data.messages, groupId);
              adminDisplay();
              availableUsersDisplay();
              socket.emit(
                "join-group",
                document.querySelector("#message-group-id").value
              );
            })
            .catch((error) => console.log(error));
        });
...
createGroupBtn.addEventListener("click", (e) => {
  ...
  if (!groupName) {
    alert("Group Name should be Null!");
    return;
  }
  axios
    .post(
